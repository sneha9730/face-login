    ...
    <nav className="w-full fixed top-0 left-0 z-50 backdrop-blur-xl bg-transparent shadow-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <Link to="/" className="flex items-center">
            ...
            <span className="ml-2 text-blue-700 font-bold text-lg">FaceAuth</span>
          </Link>
          ...
          <div className="hidden md:flex space-x-4">
            {isAuthenticated ? (
              <>
                <span className="text-blue-600">Welcome, {user?.firstName || 'User'}</span>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden px-4 pb-4 space-y-2">
          {isAuthenticated ? (
            <>
              <span className="block text-blue-600">Welcome, {user?.firstName || 'User'}</span>
            ...
            <>
              ...
              <Link to="/register" className="block text-blue-600 hover:underline">Register</Link>
            </>
          )}
        </div>
      )}
